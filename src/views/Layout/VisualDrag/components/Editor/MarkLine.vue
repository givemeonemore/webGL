...
import { sin, cos } from "@/utils/VisualDrag/translate";
import { VISUAL_DRAG_SET_SHAPE_POS_STYLE } from "@/store/types.js";
...
export default {
  ...
  computed: {
    ...mapGetters(["curComponent", "componentData"])
  },
  mounted() {
    // 监听元素移动和不移动的事件
      ...
      components.forEach(component => {
        ...
        Object.keys(conditions).forEach(key => {
          ...
          conditions[key].forEach(condition => {
            ...
            // 修改当前组件位移
            this.$store.commit(VISUAL_DRAG_SET_SHAPE_POS_STYLE, {
              key,
              value:
                rotate != 0
